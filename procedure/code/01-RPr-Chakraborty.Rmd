...
author: "Joseph Holler, Junyi Zhou, Peter Kedron, Drew An-Pham, Derrick Burt, Madeleine Gallop"
date: "`r Sys.Date()`"
output: pdf_document
editor_options:
  markdown:
...
Version 2.1 \| First Created September 21, 2023 \| Updated June 26, 2023
...
transposed_data <- filter(acs_covid, is.na(bpov_pct)) %>% st_drop_geometry() %>% t()

transposed_data %>% kable() %>% kable_styling()
...
## Write reuseable function for creating maps 

```{r missing data}
create_custom_map <- function(data, 
                              main_var, 
                              title, 
                              palette, 
                              border_color, 
    ...
    map <- tm_shape(data) +
    tm_polygons(main_var,
      title = title,
      style = style,
      border.alpha = border_alpha,
      lwd = lwd,
      palette = palette,
      popus.vars = popus_vars,
      colorNA = NULL,
      labels = labels,
      midpoint = midpoint
    ) +
    tm_shape(state) +
    tm_borders(border_color, lwd = .5) +
    tm_layout(
...
tm_covid_rates <- create_custom_map(
  data = acs_covid,
  main_var = "covid_rate",
...
tm_disability_rates <- create_custom_map(
  data = acs_covid,
  main_var = "dis_pct",
  ...
  border_alpha = 0.2,
  lwd = 0.2,
  popus_vars = c("fips", "clusterID"),  # Replace with your desired popup variables
  legend_position = c("left", "bottom"),
  legend_title_size = 0.8,
  legend_text_size = 0.5,
  style = "quantile"  # Specify the style parameter here
)
...
This also confirms that our reproduced dependent variable is identical to the original dependent variable with the exception of these thirteen counties.
...
Two correlations change direction, with both *Native American* race (illustrated in scatter plot below) and *Female* households switching from positive correlations to negative correlations.
Instabilities between the parametric and non-parametric correlations arise from variables with very skewed distributions and/or weak correlations at the county level.
...
tm_spatialepi_clusters <- create_custom_map(
  data = acs_covid,
  main_var = "isCluster",
  ...
  legend_title_size = 0.8,
  legend_text_size = 0.5,
  style = "quantile"  # Specify the style parameter if needed
)
...
tm_spatialepi_local_risk_class <- create_custom_map(
  data = acs_covid,
  main_var = "loc_class",
  ...
  lwd = 0.2,
  palette = "Oranges",
  style = "cat",  # Style parameter for categorical data
  popus_vars = c("fips", "clusterID"),  # Specify popup variables if needed
  legend_position = c("left", "bottom"),
  legend_title_size = 0.8,
  legend_text_size = 0.5,
...
tm_spatialepi_cluster_risk_class <- create_custom_map(
  data = acs_covid,
  main_var = "cluster_class",
...
The maps and cross tabulation above indicate that there are important differences between the SaTScan and SpatialEpi computational environments for calculating secondary clusters.
...
A separate hypothesis was formulated for each type of sub categorization of PwDs, numbered H2.1 through H2.5 in Table 4.
...
```{r map-weights}
tm_gee_weights <- create_custom_map(
  data = gee_resid,
  main_var = "w",
  ...
  border_alpha = 0.02,
  lwd = 0.2,
  style = "jenks",  # Style parameter for data classification
  popus_vars = NULL,  # Specify popup variables if needed
  legend_position = c("left", "bottom"),
  legend_title_size = 0.8,
  legend_text_size = 0.5
)
...
# Emily and Madeleine's Integrated Discussion

In this section, we provide a comprehensive overview of our reproduction analysis, focusing on key findings, methodological adjustments, and implications for the original study's contributions to scientific knowledge."
...
Upon revisiting the report in Fall 2023, certain sections of the study were reorganized and clarified. 
The conclusion section was created and closely edited to provide the reader with a quick and interpretable reference point to learn about what in the study was effective and how it contributes to scientific knowledge. 
The maps were made more efficient through the creation of the function 'create_custom_map', which was reused in most of the maps in this reproduction study. More work can still be done to apply this function to the SaTScan maps as well as the Race Model Residual map. 
Finally, the 'Rational for Updated Report' section was moved to the end of the study, and a couple aesthetic changes were implemented, including adjusting the color ramp on the final choropleth map and transposing the missing data table with Kable and/or KableExtra functions. 
All in all, the final changes implemented in this report serve to make it more readable, visually cohesive, and concise. 

# Conclusions

Our attempt to reproduce the original study was partially successful, leading to similar, but inexact results compared to the original study.
Some of this variability can be attributed to differences in the computational environments and analytical approaches.

However, it's important to highlight that our reanalysis of the study and the introduction of certain parameter changes added a layer of complexity to our findings. 
Despite these variations, our results provide support for the conclusions drawn in the original study.
Nevertheless, our reproduction study underscores the critical importance of replication and the need for a comprehensive review of research design elements, including internal validity and sensitivity to key parameters.

In line with the original study, we concur with the author's cautious interpretation, emphasizing the associations at the county level and the necessity for additional data and analysis. 
It's worth noting that our replication study identified several sources of uncertainty, including ecological fallacy, scale dependency, the modifiable areal unit problem, variable measurement, and spatial dependency.

There is a risk of ecological fallacy with this research design, as interpreting county-level statistics as definitive proof of indevidual-level relationships, especially in counties with small populations, can be problematic, especially given that counties are not weighted by population. Therefore, in this analysis a few people with disabilities in a rural county are collectively weighted as one observation with equal weight to many people with disabilities in an urban county as another observation. This introduces potential scale-dependent biases. 

The modifiable areal unit problem (MAUP) further complicates the analysis, particularly in lgiht of varying county population sizes at the county level, which all carry equal weights in the analysis.

Finally, we must acknowledge the challenges posed by the testing and reporting of COVID-19 cases and uncertainty in the ACS data, which can influence cross-tabulations of demographic characteristics.

Our study attempted to mitigate measurement uncertainty by aggregating data across multiple years and using relatively large geographic units (counties).
We also considered spatial dependence by clustering county observations based on state and COVID risk classification. 
However, our approach does not account for spatial dependence between counties in different clusters, even if they are adjacent.

In summary, while our replication results exhibited some disparities from the original study due to methodological and computational differences, they still indicate that individuals with disabilities, particularly those from racial/ethnic minority backgrounds and in poverty, may face heightened vulnerabilities.
We echo the original study's emphasis on the county-level nature of these associations and the need for further data and research.
It is crucial to recognize that the relationships observed at the county level may not necessarily hold at the individual level. 
Further investigations are warranted, especially at finer geographic scales, to reliably infer the relationships between minority individuals with disabilities and COVID-19 morbidity and mortality.
...
We also discovered an improved method for comparing the results of generalized linear models by plotting all of the model results on a dot-whisker graph using the dot whisker package.
